"""The SCOPED DiDip layout serving microservice (``sly``).

Launch (if not running):  ddpa_scoped_layout_serve
...
from .scoped_config import MsScopedLayout


@scoped_ms
class ScopedLayoutMicroservice(SharedIndexMicroservice):
    """Layout regions browsable by class, scoped to the active basket."""

    config_class = MsScopedLayout
    GLOBAL_ROUTE_PREFIX = "sly"
    LAUNCH_CMD = "ddpa_scoped_layout_serve"
    VIEWS = ("charter", "root")
    ...
    def register_routes(self):
        ...
        def classes():
            ...
            return self.render('sly_classes.html', rows=rows, summary=summary,
        ...
        def classitems(classname, skip=0, itemcount=None):
            ...
            return self.render('sly_classitems.html', classname=classname, items=items,
        ...
        def charter(md5):
            ...
            return self.render('sly_charter.html', charter_md5=md5, images=images,
                               sibling_md5=md5)
